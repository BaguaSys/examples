...
def main():
    ...
    # allgather
    send_tensor = torch.rand(4, dtype=torch.float32).cuda()
    recv_tensor_bagua = torch.zeros(4 * bagua.get_world_size(), dtype=torch.float32).cuda()
    recv_tensors = [torch.zeros(4, dtype=torch.float32).cuda() for i in range(bagua.get_world_size())]
    ...
    # reduce_scatter
    send_tensors = [torch.rand(4, dtype=torch.float32).cuda() for i in range(bagua.get_world_size())]
    recv_tensor = torch.zeros(4, dtype=torch.float32).cuda()
