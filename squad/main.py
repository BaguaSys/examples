...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    if not os.path.exists(args.output_dir) and bagua.get_rank() == 0:
...
def main():
    ...
    if not args.distributed or args.no_cuda:
        ...
        device = torch.device("cuda", bagua.get_local_rank())
    ...
    if is_main_process(bagua.get_rank()):
    ...
    if args.do_train and bagua.get_rank() == 0:
    ...
    results = {}
    if args.do_eval and bagua.get_rank() == 0:
        if args.do_train:
